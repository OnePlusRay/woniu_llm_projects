...
# 加载模型
model = AutoModelForCausalLM.from_pretrained(model_path, device_map={'':"cuda"}, torch_dtype=torch.bfloat16, trust_remote_code=True).eval()
...
for idx, item in enumerate(tqdm1(data)):
    ...
    context = index.query(question, k)  # 返回相似度最高的 k 个文档
