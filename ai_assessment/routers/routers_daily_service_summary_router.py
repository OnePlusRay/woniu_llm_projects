...
def transform_raw_conversation_2_str(raw_conversation):
    ...
    wechat_raw_conversations = raw_conversation["wechatContentList"]
    # wechat_conversation = concat_conversation(wechat_raw_conversation)
    for index, wechat_raw_conversation in enumerate(wechat_raw_conversations):
        wechat_conversation = concat_conversation(wechat_raw_conversation['wechatContent'])
    
    final_conversation = "\n\n".join(call_conversations) + "\n\n微信聊天记录：\n" + wechat_conversation
    return final_conversation


def daily_potential_client_check_process_func(job_request:JobRequest):
