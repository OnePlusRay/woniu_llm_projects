...
def transform_raw_conversation_2_str(raw_conversation):
    service_raw_Summarys = raw_conversation["serviceSummarys"]
    serviceSummarys = []
    for index, service_raw_Summary in enumerate(service_raw_Summarys):
        serviceSummarys.append(f"第{index+1}个服务：\n" + concat_conversation(service_raw_Summary['summary']))

    policy_raw_Records = raw_conversation["policyRecords"]
    policyRecords = []
    for index, policy_raw_Record in enumerate(policy_raw_Records):
        policyRecords.append(f"投保情况：\n")
        policyRecords.append(concat_conversation(policy_raw_Record['productName'])+"\n")#  三个参数 "productName","productType","fee"
    
    complaint_raw_Records = raw_conversation["complaintRecords"]
    complaintRecords = []
    for index, complaint_raw_Record in enumerate(complaint_raw_Records):
        complaintRecords.append(f"第{index+1}个投诉：\n" + concat_conversation(complaint_raw_Record['content'])) 
    
    final_conversation = "\n\n".join(serviceSummarys) + "\n\n" .join(policyRecords) + "\n\n" .join(complaintRecords)
    return final_conversation


def month_potential_client_check_process_func(job_request:JobRequest):
