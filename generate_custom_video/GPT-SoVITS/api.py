...
class SubtitleGenerator:
    ...
    def format_time(self, start_time, init_time):
    ...
    def split_text(self, text, min_len=8, max_len=16, max_boundary=24):
        # Tokenize text based on Chinese characters and punctuation marks.
        tokens = re.findall(r'[\w]+|[。，、：；？！]', text)
        
        # Result list to hold the final segments
        result = []
        current_segment = []
        current_length = 0
        
        def clean_segment(segment):
            """Remove punctuation from the start and end of the segment."""
        ...
        for token in tokens:
            ...
            # Check if the current segment should be split
            if current_length >= min_len or token == '。':
                if current_length <= max_len:
                    clean_segment(current_segment)
                    result.append(''.join(current_segment))
                    current_segment = []
                    current_length = 0
                elif current_length <= max_boundary:
                    clean_segment(current_segment)
                    result.append(''.join(current_segment))
                    current_segment = []
                    current_length = 0
    ...
    def get_audio_duration(self, ref_wav_path, prompt_text, prompt_language, text, text_language):
        t0 = ttime()
        ...
        for text in texts:
            ...
            t4 = ttime()
            audio_concat = np.concatenate(audio_opt, 0)
            audio_bytes = pack_audio(audio_bytes, (audio_concat * 32768).astype(np.int16), hps.data.sampling_rate)
            audio_duration.append(audio_concat.shape[0] / float(hps.data.sampling_rate))

        if not stream_mode == "normal": 
            if media_type == "wav":
                audio_bytes = pack_wav(audio_bytes,hps.data.sampling_rate)
            return audio_bytes.getvalue(), audio_duration
        
    def generate_srt(self, audio_duration, text, init_time):
        srt_entries = []
        ...
        for i, text in enumerate(processed_texts):
            if only_punc(text):
                continue
            end_time = start_time + datetime.timedelta(seconds=audio_duration[i])
            
            # Split the text into lines of maximum 8 characters
            lines = []
            for j in range(0, len(text), 8):
                lines.append(text[j:j+8])
            
            srt_entry = f"{entry_number}\n{self.format_time(start_time, init_time)} --> {self.format_time(end_time, init_time)}\n"
            srt_entry += "\n".join(lines) + "\n\n"
...
@app.post("/get_srt")
async def srt_endpoint(request: Request):
    # 获得参数
    json_post_raw = await request.json()
    ...
    unique_id = json_post_raw.get("unique_id")
    if (
            refer_wav_path == "" or refer_wav_path is None
            or prompt_text == "" or prompt_text is None
            or prompt_language == "" or prompt_language is None
    ...
    # 生成字幕
    subtitle_generator = SubtitleGenerator(os.path.join(srt_path,f"{unique_id}.srt"))

    text = subtitle_generator.split_text(text)
    # 处理TTS和获取音频时长
    audio_bytes, audio_duration = subtitle_generator.get_audio_duration(refer_wav_path, prompt_text, prompt_language, text, text_language)
    # 处理文本和生成SRT
    init_time = datetime.datetime.now()  # 这里的时间应与音频的起始时间同步
    srt_content = subtitle_generator.generate_srt(audio_duration, text, init_time)

    # 保存srt文件
    subtitle_generator.save_srt(srt_content)
    
    # 可选择的：将SRT文件内容作为响应的一部分返回
